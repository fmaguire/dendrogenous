...
class TestSettings(BaseTestCase):
    ...
    def test_clean_init(self):
        ...
        """
        needed_settings = io.StringIO('{"output_dir": "test_out", "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {"a": 4}}')
    ...
    def test__check_mandatory_fields_typerror_if_type_wrong(self):
        ...
        """
        needed_settings = io.StringIO('{"genome_list": "", "genome_dir": "foo", "dbconfig": {"a": 4}}')
        with self.assertRaises(TypeError):
            test_settings = dg.settings.Settings(needed_settings)

        needed_settings = io.StringIO('{"genome_list": ["a", "b", "c"], "genome_dir": {}, "dbconfig": {"a": 4}}')
        with self.assertRaises(TypeError):
            test_settings = dg.settings.Settings(needed_settings)

        needed_settings = io.StringIO('{"genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": []}')
        with self.assertRaises(TypeError):
            test_settings = dg.settings.Settings(needed_settings)
    ...
    def test__collate_default_and_user_settings_raises_TypeError_if_new_type_wrong(self):
        ...
        """
        needed_settings = io.StringIO('{"output_dir": [], "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {"a": 4}}')
        with self.assertRaises(TypeError):
            test_settings = dg.settings.Settings(needed_settings)

        needed_settings = io.StringIO('{"output_dir": "test_out", "blast_settings": [], "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {"a": 4}}')
        with self.assertRaises(TypeError):
            test_settings = dg.settings.Settings(needed_settings)

        needed_settings = io.StringIO('{"output_dir": "test_out", "binary_paths": "", "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {"a": 4}}')
        with self.assertRaises(TypeError):
            test_settings = dg.settings.Settings(needed_settings)
    ...
    def test__collate_default_and_user_settings_raises_ValueError_if_wrong_user_input_field_identified(self):
        ...
        """
        needed_settings = io.StringIO('{"output_dir": "test_out", "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {"a": 4}}, "foo": ""')
        with self.assertRaises(ValueError):
            test_settings = dg.settings.Settings(needed_settings)

        needed_settings = io.StringIO('{"output_dir": "test_out", "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {"a": 4}}, "bar": {}')
        with self.assertRaises(ValueError):
            test_settings = dg.settings.Settings(needed_settings)

        needed_settings = io.StringIO('{"output_dir": "test_out", "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {"a": 4}}, "baz": []')
        with self.assertRaises(ValueError):
            test_settings = dg.settings.Settings(needed_settings)
    ...
    def test_db_config_success(self):
        ...
        setting_str = '{{"output_dir": "test_out", "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {0}}}'.format(str(secret_config).replace('\'', '\"'))
    ...
    def test_db_config_fail(self):
        ...
        """
        needed_settings = io.StringIO('{"output_dir": "test_out", "genome_list": ["a", "b", "c"], "genome_dir": "foo", "dbconfig": {}}')
        test_settings = dg.settings.Settings(needed_settings)
        with self.assertRaises(ValueError):
            foo = test_settings.dbconfig
